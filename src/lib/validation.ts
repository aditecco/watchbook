...
export class InputValidator {
  ...
  static validatePassword(password: string): { isValid: boolean; sanitized: string } {
    ...
    return {
      isValid: sanitized.length >= minLength && hasUpperCase && hasLowerCase && hasNumbers,
      sanitized
    };
  }

  // Validate and sanitize search terms
  static validateSearchTerm(term: string): { isValid: boolean; sanitized: string } {
  ...
  static validateNoteContent(content: string): { isValid: boolean; sanitized: string } {
  ...
  static validateTagName(name: string): { isValid: boolean; sanitized: string } {
    const sanitized = this.sanitizeText(name.trim());
    const validChars = /^[a-zA-Z0-9\s\-_]+$/;

    return {
      isValid: sanitized.length >= 1 && sanitized.length <= 50 && validChars.test(sanitized),
      sanitized
    };
  }
  ...
  static validateRating(rating: number): { isValid: boolean; sanitized: number } {
  ...
  static validateImdbId(imdbId: string): { isValid: boolean; sanitized: string } {
  ...
  static validateString(value: string, minLength: number = 1, maxLength: number = 255): { isValid: boolean; sanitized: string } {
  ...
  static validateNumber(value: number, min: number = 0, max: number = Number.MAX_SAFE_INTEGER): { isValid: boolean; sanitized: number } {
  ...
  static validateStatus(status: string): { isValid: boolean; sanitized: string } {
  ...
  static validateContentType(type: string): { isValid: boolean; sanitized: string } {
...
export class RateLimiter {
  ...
  static checkLimit(key: string, maxAttempts: number = 5, windowMs: number = 60000): boolean {
    const now = Date.now();
